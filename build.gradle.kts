...
base {
    archivesName.set("${prop("mod.id")}-fabric")
}
...
tasks.processResources {
    applyProperties(project, listOf("fabric.mod.json", "${prop("mod.id")}-common.mixin.json", "${prop("mod.id")}-fabric.mixin.json"))
}
...
tasks.register<Copy>("buildAndCollect") {
    ...
    into(rootProject.layout.buildDirectory.file("libs/${prop("mod.version")}/fabric"))
    dependsOn("build")
}
