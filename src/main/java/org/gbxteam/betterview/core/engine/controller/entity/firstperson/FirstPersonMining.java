...
public class FirstPersonMining {
    ...
    /** BFP Mining Animation Pipeline - GBXTeam */
    ...
    public static PoseFunction<LocalSpacePose> constructMiningPoseFunction(
        ...
        PoseFunction<LocalSpacePose> stateMachine = PoseManager.builder(context -> MINING_IDLE_STATE)
    ...
    public static Function<PoseTickEvaluationContext, Float> getMiningPlayRateFunction(float baseMultiplier) {
        return context -> baseMultiplier * BFPMain.CONFIG.data().firstPersonPlayer.miningAnimationSpeedMultiplier;
    }
    ...
    public static PoseFunction<LocalSpacePose> constructPickaxeMiningPoseFunction(PoseFunction<LocalSpacePose> basePose) {
        return FirstPersonMining.constructMiningPoseFunction(
                SequenceEvaluatorFunction.builder(FirstPersonMovementFlows.HAND_TOOL_POSE).build(),
                SequencePlayerFunction.builder(FirstPersonMovementFlows.HAND_TOOL_PICKAXE_MINE_SWING)
                        .setLooping(true)
                        .setResetStartTimeOffset(TimeSpan.of60FramesPerSecond(10))
                        .setPlayRate(getMiningPlayRateFunction(1.1f))
                        .build(),
                SequencePlayerFunction.builder(FirstPersonMovementFlows.HAND_TOOL_RAISE)
    ...
    public static PoseFunction<LocalSpacePose> constructAxeMiningPoseFunction() {
        return FirstPersonMining.constructMiningPoseFunction(
                SequenceEvaluatorFunction.builder(FirstPersonMovementFlows.HAND_TOOL_POSE).build(),
                SequencePlayerFunction.builder(FirstPersonMovementFlows.HAND_TOOL_AXE_MINE_SWING)
                        .setLooping(true)
    ...
    public static PoseFunction<LocalSpacePose> constructShovelMiningPoseFunction() {
        return FirstPersonMining.constructMiningPoseFunction(
                SequenceEvaluatorFunction.builder(FirstPersonMovementFlows.HAND_TOOL_POSE).build(),
                SequencePlayerFunction.builder(FirstPersonMovementFlows.HAND_TOOL_SHOVEL_MINE_SWING)
                        .setLooping(true)
                        .setResetStartTimeOffset(TimeSpan.of60FramesPerSecond(0))
                        .setPlayRate(getMiningPlayRateFunction(1.1f))
                        .build(),
    ...
    public static PoseFunction<LocalSpacePose> constructEmptyHandMiningPoseFunction() {
        ...
        PoseFunction<LocalSpacePose> mineSwingPoseFunction = SequencePlayerFunction.builder(FirstPersonMovementFlows.HAND_EMPTY_MINE_SWING)
                .setLooping(true)
        ...
        PoseFunction<LocalSpacePose> mineFinishPoseFunction = SequencePlayerFunction.builder(FirstPersonMovementFlows.HAND_TOOL_RAISE)
                .setResetStartTimeOffset(TimeSpan.of60FramesPerSecond(4))
    ...
    public static PoseFunction<LocalSpacePose> constructWithPunchMiningPoseFunction(PoseFunction<LocalSpacePose> inputPose) {
        ...
        PoseFunction<LocalSpacePose> swingAPose = SequencePlayerFunction.builder(FirstPersonMovementFlows.HAND_EMPTY_MINE_SWING)
                .setLooping(false)
                .setPlayRate(context -> 1.3f * BFPMain.CONFIG.data().firstPersonPlayer.miningAnimationSpeedMultiplier)
                .build();
        PoseFunction<LocalSpacePose> swingBPose = MirrorFunction.of(swingAPose);
        PoseFunction<LocalSpacePose> finishPose = SequencePlayerFunction.builder(FirstPersonMovementFlows.HAND_EMPTY_MINE_FINISH).build();

        PoseFunction<LocalSpacePose> miningStateMachine;
        miningStateMachine = PoseManager.builder(context -> PUNCH_MINING_IDLE_STATE)
                .resetsUponRelevant(true)
