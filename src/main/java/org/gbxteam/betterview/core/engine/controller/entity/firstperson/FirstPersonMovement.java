...
import org.gbxteam.betterview.core.engine.controller.entity.firstperson.handpose.FirstPersonMap;
...
public class FirstPersonMovement {
    ...
    /** BFP Movement Animation Pipeline - GBXTeam */
    ...
    public static boolean isFalling(StateTransitionContext context) {
        boolean isGrounded = context.getDriverValue(FirstPersonDrivers.IS_ON_GROUND);
        float verticalSpeed = context.getDriverValue(FirstPersonDrivers.VERTICAL_MOVEMENT_SPEED);
        // Only trigger falling state if we are actually moving downwards.
        // This prevents step-assist or sprint-jumping over flat terrain lag 
        // from triggering a micro falling/landing cycle that freezes the arms.
        return !isGrounded && verticalSpeed < -0.1f;
    }
    ...
    public static PoseFunction<LocalSpacePose> constructAdditiveMovementPoseFunction(CachedPoseContainer cachedPoseContainer) {
        ...
        additiveReferencePose = SequenceEvaluatorFunction.builder(FirstPersonMovementFlows.GROUND_MOVEMENT_POSE).build();
    ...
    public static PoseFunction<LocalSpacePose> constructWithOverridingMovementAnimations(PoseFunction<LocalSpacePose> inputPose) {
        PoseFunction<LocalSpacePose> swimmingPoseFunction = SequencePlayerFunction.builder(FirstPersonMovementFlows.OVERRIDING_MOVEMENT_SWIMMING)
                .setPlayRate(1.2f)
        ...
        overridingMovementStateMachine = PoseManager.builder(FirstPersonMovement::getOverridingMovementEntryState)
    ...
    public static float getWalkingAnimationPlayRate(DriverGetter context) {
        // BFP Frame-Drop Fix: By using the dynamically smoothed walk speed, we ensure
        // single-frame stutters (which reset vanilla speed to 0.0) don't violently freeze the animation.
        float walkSpeed = context.getDriverValue(FirstPersonDrivers.SMOOTHED_WALK_SPEED);
        ...
        if (isGrounded) {
            ...
            // Keep the animation playing at a reduced rate when briefly
            // off the ground (slopes, uneven terrain) to prevent stuttering
            return walkPlayRate * 0.5f;
        }
    }
    ...
    public static PoseFunction<LocalSpacePose> constructWalkingStateMachine() {
        PoseFunction<LocalSpacePose> idleAnimationPlayer = BlendPosesFunction.builder(
                ...
                .addBlendInput(SequencePlayerFunction.builder(FirstPersonMovementFlows.GROUND_MOVEMENT_IDLE)
                        .setLooping(true)
                        .build(),
                        context -> 0.8f)
                .build();
        PoseFunction<LocalSpacePose> stoppingPoseFunction = SequencePlayerFunction.builder(FirstPersonMovementFlows.GROUND_MOVEMENT_WALK_TO_STOP).setPlayRate(0.6f).build();

        PoseFunction<LocalSpacePose> walkingPoseFunction = SequencePlayerFunction
                .builder(FirstPersonMovementFlows.GROUND_MOVEMENT_WALKING)
        ...
        PoseFunction<LocalSpacePose> sprintingPoseFunction = SequencePlayerFunction
                .builder(FirstPersonMovementFlows.GROUND_MOVEMENT_SPRINTING)
                .setPlayRate(FirstPersonMovement::getWalkingAnimationPlayRate)
                .setLooping(true)
                .build();

        sprintingPoseFunction = BlendPosesFunction.builder(walkingPoseFunction)
                .addBlendInput(sprintingPoseFunction, context -> BFPMain.CONFIG.data().firstPersonPlayer.runningArmSwingIntensity)
                .build();
    ...
    public static PoseFunction<LocalSpacePose> constructAdditiveCrouchStateMachine() {
        PoseFunction<LocalSpacePose> standingPoseFunction = SequenceEvaluatorFunction.builder(FirstPersonMovementFlows.GROUND_MOVEMENT_POSE).build();
        PoseFunction<LocalSpacePose> crouchPoseFunction = SequenceEvaluatorFunction.builder(FirstPersonMovementFlows.GROUND_MOVEMENT_CROUCH_OUT)
                .evaluatesPoseAt(TimeSpan.ZERO)
                .build();
        PoseFunction<LocalSpacePose> crouchInPoseFunction = SequencePlayerFunction.builder(FirstPersonMovementFlows.GROUND_MOVEMENT_CROUCH_IN)
                .setPlayRate(0.8f)
                .build();
        PoseFunction<LocalSpacePose> crouchOutPoseFunction = SequencePlayerFunction.builder(FirstPersonMovementFlows.GROUND_MOVEMENT_CROUCH_OUT)
                .setPlayRate(0.9f)
                .build();

        PoseFunction<LocalSpacePose> crouchStateMachine = PoseManager.builder(FirstPersonMovement::getCrouchingEntryState)
                .resetsUponRelevant(true)
    ...
    public static PoseFunction<LocalSpacePose> makeAdditiveMovementPose(PoseFunction<LocalSpacePose> inputPose) {
        PoseFunction<LocalSpacePose> basePose = SequenceEvaluatorFunction.builder(FirstPersonMovementFlows.GROUND_MOVEMENT_POSE).build();
    ...
    public static PoseFunction<LocalSpacePose> constructFallingPose(boolean isLong) {
        ...
        if (isLong) {
            pose = BlendedSequencePlayerFunction.builder(FirstPersonDrivers.VERTICAL_MOVEMENT_SPEED)
                    .addEntry(0f, FirstPersonMovementFlows.GROUND_MOVEMENT_FALLING_IN_PLACE, 0)
                    .addEntry(-1f, FirstPersonMovementFlows.GROUND_MOVEMENT_FALLING_LOOP, 1.4f)
                    .addEntry(-2.5f, FirstPersonMovementFlows.GROUND_MOVEMENT_FALLING_LOOP, 2)
                    .build();
        } else {
            pose = SequenceEvaluatorFunction.builder(FirstPersonMovementFlows.GROUND_MOVEMENT_FALLING_IN_PLACE).build();
        }
    ...
    public static PoseFunction<LocalSpacePose> constructWithFallingStateMachine(CachedPoseContainer cachedPoseContainer, PoseFunction<LocalSpacePose> standingPose) {
        PoseFunction<LocalSpacePose> jumpPoseFunction = SequencePlayerFunction.builder(FirstPersonMovementFlows.GROUND_MOVEMENT_JUMP).build();
        PoseFunction<LocalSpacePose> jumpRunningPoseFunction = SequencePlayerFunction.builder(FirstPersonMovementFlows.GROUND_MOVEMENT_JUMP_RUNNING).build();
        ...
        PoseFunction<LocalSpacePose> landPoseFunction = SequencePlayerFunction.builder(FirstPersonMovementFlows.GROUND_MOVEMENT_LAND)
                .build();
        PoseFunction<LocalSpacePose> softLandPoseFunction = BlendPosesFunction.builder(SequenceEvaluatorFunction.builder(FirstPersonMovementFlows.GROUND_MOVEMENT_POSE).build())
                .addBlendInput(SequencePlayerFunction.builder(FirstPersonMovementFlows.GROUND_MOVEMENT_LAND).setPlayRate(1f).build(), context -> 0.5f)
                .build();
        ...
        fallingStateMachine = PoseManager.builder(FirstPersonMovement::getFallingEntryState)
                .resetsUponRelevant(true)
    ...
    public static PoseFunction<LocalSpacePose> constructWithUnderwaterStateMachine(PoseFunction<LocalSpacePose> inputPose) {
        Identifier groundMovementSwimmingSequence = FirstPersonMovementFlows.GROUND_MOVEMENT_SWIMMING_IDLE;
        PoseFunction<LocalSpacePose> treadingUnderwaterPoseFunction = BlendedSequencePlayerFunction.builder(FirstPersonDrivers.VERTICAL_MOVEMENT_SPEED)
                ...
                .build();
        PoseFunction<LocalSpacePose> landUnderwaterPoseFunction = BlendPosesFunction.builder(SequenceEvaluatorFunction.builder(FirstPersonMovementFlows.GROUND_MOVEMENT_POSE).build())
                .addBlendInput(SequencePlayerFunction.builder(FirstPersonMovementFlows.GROUND_MOVEMENT_LAND)
                        .setPlayRate(0.7f)
                        .build(),
                        context -> 0.4f)
                .build();
        PoseFunction<LocalSpacePose> onGroundUnderwaterPoseFunction = BlendedSequencePlayerFunction.builder(FirstPersonDrivers.MODIFIED_WALK_SPEED)
                .setResetStartTimeOffset(TimeSpan.of30FramesPerSecond(5))
                .addEntry(0f, FirstPersonMovementFlows.GROUND_MOVEMENT_POSE, 0.1f)
                .addEntry(0.3f, FirstPersonMovementFlows.GROUND_MOVEMENT_WALKING, 0.8f)
                .build();

        PoseFunction<LocalSpacePose> underwaterStateMachine;
        underwaterStateMachine = PoseManager.builder(FirstPersonMovement::getUnderwaterEntryState)
                .resetsUponRelevant(true)
    ...
    public static PoseFunction<LocalSpacePose> constructWithMountStateMachine(PoseFunction<LocalSpacePose> inputPose) {
        ...
        mountEnterPoseFunction = SequencePlayerFunction.builder(FirstPersonMovementFlows.GROUND_MOVEMENT_MOUNT_ENTER)
                .setResetStartTimeOffset(TimeSpan.of60FramesPerSecond(6))
                .build();
        mountedPoseFunction = SequenceEvaluatorFunction.builder(FirstPersonMovementFlows.GROUND_MOVEMENT_POSE).build();

        PoseFunction<LocalSpacePose> mountStateMachine;
        mountStateMachine = PoseManager.builder(FirstPersonMovement::getMountEntryState)
