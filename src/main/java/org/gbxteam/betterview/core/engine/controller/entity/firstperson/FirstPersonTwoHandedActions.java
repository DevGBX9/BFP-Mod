...
import org.gbxteam.betterview.core.skeleton.rig.BlendProfile;
import org.gbxteam.betterview.core.logic.states.LocalSpacePose;
import org.gbxteam.betterview.core.logic.poses.MirrorFunction;
...
public class FirstPersonTwoHandedActions {
    ...
    /** BFP Two-Handed Action Pipeline - GBXTeam */
    public static PoseFunction<LocalSpacePose> constructPoseFunction(PoseFunction<LocalSpacePose> normalPoseFunction, CachedPoseContainer cachedPoseContainer) {
        PoseManager.Builder builder = PoseManager.builder(FirstPersonTwoHandedActions::getTwoHandedEntryState)
    ...
    public static void defineBowStatesForHand(PoseManager.Builder stateMachineBuilder, InteractionHand hand) {
        ...
        PoseFunction<LocalSpacePose> pullPoseFunction = SequencePlayerFunction.builder(FirstPersonMovementFlows.HAND_BOW_PULL)
                ...
                .build();
        PoseFunction<LocalSpacePose> releasePoseFunction = SequencePlayerFunction.builder(FirstPersonMovementFlows.HAND_BOW_RELEASE)
                .build();
    ...
    public static void defineCrossbowStatesForHand(PoseManager.Builder stateMachineBuilder, InteractionHand hand) {
        InteractionHand oppositeHand = hand == InteractionHand.MAIN_HAND ? InteractionHand.OFF_HAND : InteractionHand.MAIN_HAND;
        String crossbowReloadState = getCrossbowReloadState(hand);
        String crossbowFinishReloadState = getCrossbowFinishReloadState(hand);

        PoseFunction<LocalSpacePose> crossbowReloadPoseFunction = SequencePlayerFunction.builder(FirstPersonMovementFlows.HAND_CROSSBOW_RELOAD)
                .setPlayRate(context -> context.getDriverValue(FirstPersonDrivers.CROSSBOW_RELOAD_SPEED))
                .build();
        PoseFunction<LocalSpacePose> crossbowFinishReloadPoseFunction = SequencePlayerFunction.builder(FirstPersonMovementFlows.HAND_CROSSBOW_RELOAD_FINISH)
                .build();
