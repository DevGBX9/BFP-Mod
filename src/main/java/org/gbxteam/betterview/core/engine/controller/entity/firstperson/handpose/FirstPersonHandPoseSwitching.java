...
//        }
    ...
    public static PoseFunction<LocalSpacePose> constructPoseFunction(CachedPoseContainer cachedPoseContainer, InteractionHand hand) {
        ...
        PoseManager.Builder stateMachineBuilder = PoseManager.builder(context -> getEntryHandPoseState(context, hand));
        ...
        // Forcefully stripped pre-baked camera animations from all hand interactions
        // so we exclusively use the authentic Camera Overhaul ScreenShakes instead.
        pose = BlendPosesFunction.builder(pose)
                .addBlendInput(
                        EmptyPoseFunction.of(),
                        context -> 1.0f, // 100% override the camera joint with nothing
    ...
    public static void defineStatesForHandPose(
            ...
            InteractionHand hand,
            PoseManager.Builder stateMachineBuilder,
    ...
    public static void defineExtraStates(PoseManager.Builder stateMachineBuilder, InteractionHand hand) {
        ...
        useLastItemPoseFunction = SequencePlayerFunction.builder(FirstPersonMovementFlows.HAND_TOOL_USE).isAdditive(true, SequenceReferencePoint.END).build();
        ...
        dropLastItemPoseFunction = SequencePlayerFunction.builder(FirstPersonMovementFlows.HAND_TOOL_USE).isAdditive(true, SequenceReferencePoint.END).build();
        dropLastItemPoseFunction = ApplyAdditiveFunction.of(emptyHandPose.constructBasePoseFunction(hand), dropLastItemPoseFunction);

        PoseFunction<LocalSpacePose> throwingTridentPoseFunction = SequencePlayerFunction.builder(FirstPersonMovementFlows.HAND_TRIDENT_RELEASE_THROW).build();
