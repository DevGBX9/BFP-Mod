...
public class SpringDriver<D> extends VariableDriver<D> {
    ...
    public void reset() {
        super.reset();
        this.previousTargetValue = this.baseValueProvider.get();
        this.currentTargetValue = this.baseValueProvider.get();
    }
    ...
    public D getInterpolatedValue(float partialTicks) {
        ...
        if (this.returnsDelta) {
            D targetInterpolatedValue = this.blendFunction.interpolate(this.previousTargetValue, this.currentTargetValue, partialTicks);
    ...
    public void tick() {
        ...
        D displacement = this.addition.apply(this.liveTickValue, this.multiplication.apply(this.currentTargetValue, -1f));
        ...
        this.liveTickValue = this.addition.apply(this.liveTickValue, this.velocity);
    }
}
