...
public class BlendedSequencePlayerFunction extends TimeBasedPoseFunction<LocalSpacePose> {
    ...
    public @NotNull LocalSpacePose compute(PoseCalculationContext context) {
        ...
        if (floorEntry == null)
            return MovementFlow.samplePose(context.jointSkeleton(), ceilingEntry.getValue().animationSequence(), time, true);
        if (ceilingEntry == null)
            return MovementFlow.samplePose(context.jointSkeleton(), floorEntry.getValue().animationSequence(), time, true);
        ...
        LocalSpacePose floorPose = MovementFlow.samplePose(context.jointSkeleton(), floorEntry.getValue().animationSequence(), time, true);
        LocalSpacePose ceilingPose = MovementFlow.samplePose(context.jointSkeleton(), ceilingEntry.getValue().animationSequence(), time, true);
