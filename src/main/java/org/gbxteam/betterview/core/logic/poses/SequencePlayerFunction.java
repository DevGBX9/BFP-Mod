...
public class SequencePlayerFunction extends TimeBasedPoseFunction<LocalSpacePose> implements AnimationPlayer {
    ...
    public @NotNull LocalSpacePose compute(PoseCalculationContext context) {
        LocalSpacePose pose = MovementFlow.samplePose(
                context.jointSkeleton(),
                this.animationSequence,
                this.getInterpolatedTimeElapsed(context),
                this.isLooping
        );
        MovementFlow sequence = BFPResources.getOrThrowMovementFlow(this.animationSequence);

        if (!this.isAdditive) {
            return pose;
        }
        if (this.additiveSubtractionPose == null) {
            this.additiveSubtractionPose = MovementFlow.samplePose(
                    context.jointSkeleton(),
                    this.animationSequence,
    ...
    public void tick(PoseTickEvaluationContext context) {
        super.tick(context);
        Set<String> timeMarkersToFire = BFPResources.getOrThrowMovementFlow(this.animationSequence).getMarkersInRange(TimeSpan.ofTicks(this.ticksElapsed.getCurrentValue()), TimeSpan.ofTicks(this.ticksElapsed.getCurrentValue() + this.playRate), this.isLooping);
    ...
    public Tuple<TimeSpan, TimeSpan> getRemainingTime() {
        float lengthInTicks = BFPResources.getOrThrowMovementFlow(animationSequence).length().inTicks();
    ...
    public TimeSpan getAnimationLength() {
        return BFPResources.getOrThrowMovementFlow(this.animationSequence).length();
    }
