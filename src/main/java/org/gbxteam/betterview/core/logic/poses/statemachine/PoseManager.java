...
import org.gbxteam.betterview.core.logic.poses.TimeBasedPoseFunction;
 ...
 */
public class PoseManager extends TimeBasedPoseFunction<LocalSpacePose> {

    private static final Logger LOGGER = LogManager.getLogger("BFP/PoseManager");

    private final Map<String, StateDefinition> states;
    private final Function<DriverGetter, String> initialStateFunction;
    private final List<ProceduralLayer> proceduralBlendMatrix;
    ...
    private PoseManager(
        ...
        this.proceduralBlendMatrix = new ArrayList<>();
    ...
    public @NotNull LocalSpacePose compute(PoseCalculationContext context) {
        ...
        if(this.proceduralBlendMatrix.isEmpty()){
        ...
        // Blend the poses dynamically via procedural blending instead of strict stack layering.
        LocalSpacePose pose = layerStackPoses.get(this.proceduralBlendMatrix.getFirst().identifier);

        if (this.proceduralBlendMatrix.size() > 1) {
            for (ProceduralLayer proceduralLayer : this.proceduralBlendMatrix.subList(1, proceduralBlendMatrix.size())) {
                
                // BFP Procedural Engine: Apply Custom SmoothStep Math (3x^2 - 2x^3) to convert rigid weights into dynamic dampening algorithms
                float rawLinearWeight = proceduralLayer.weight.getInterpolatedValue(context.partialTicks());
                float proceduralWeight = (rawLinearWeight * rawLinearWeight) * (3.0f - 2.0f * rawLinearWeight); // Procedural Ease-In-Out Dampening

                pose.interpolatedByTransition(
                        layerStackPoses.get(proceduralLayer.identifier),
                        proceduralWeight, // Use mathematically dampened weight
                        proceduralLayer.entranceTransition.transition(),
                        null
                );
            }
        }
    ...
    public void tick(PoseTickEvaluationContext context) {
        ...
        // Calculate algorithmic physics on the Blend Matrix.
        this.proceduralBlendMatrix.forEach(ProceduralLayer::processMathematicalDampening);
        this.clearOverriddenPriors();
    ...
    private void takeInitialStateIfResetting(PoseTickEvaluationContext context) {
        // If the procedural matrix is empty, initialize it using the initial procedural function.
        boolean matrixIsEmpty = this.proceduralBlendMatrix.isEmpty();
        boolean hasBecomeRelevant = context.resetting();
        if (matrixIsEmpty || (hasBecomeRelevant && this.resetsUponRelevant)) {
            this.proceduralBlendMatrix.clear();
            String initialStateIdentifier = this.initialStateFunction.apply(context);
            if (this.states.containsKey(initialStateIdentifier)) {
                this.proceduralBlendMatrix.addLast(new ProceduralLayer(
                ...
                throw new IllegalStateException("Initial algorithmic node " + initialStateIdentifier + " not found to be present in the matrix");
            }
        }
    ...
    private Optional<StateTransition> testForOutboundTransition(PoseTickEvaluationContext context) {
        // Get the current active functional node from matrix
        String currentActiveStateIdentifier = this.proceduralBlendMatrix.getLast().identifier;
                ...
                .filter(stateTransition -> {
                    ...
                    if(transitionTargetIncludedInThisMachine && targetIsNotCurrentActiveState){
                        StateTransitionContext transitionContext = new StateTransitionContext(
                                ...
                                this.proceduralBlendMatrix.getLast().weight.getCurrentValue(),
                                this.proceduralBlendMatrix.getLast().weight.getPreviousValue(),
    ...
    private void takeTransition(PoseTickEvaluationContext context, StateTransition transition) {
        ...
        this.proceduralBlendMatrix.addLast(new ProceduralLayer(transition.target(), transition));
        this.resetTime();
    }

    private void clearOverriddenPriors() {
        // Iterate through the procedural matrix sequentially top-down.
        // If a dimensional vector fully overrides those below it, sever the mathematical connections to prevent wasted CPU ticks.
        boolean higherNodeIsFullyOverriding = false;
        List<ProceduralLayer> inactiveDimensions = new ArrayList<>();
        for (ProceduralLayer proceduralLayer : this.proceduralBlendMatrix.reversed()) {
            if (higherNodeIsFullyOverriding) {
                inactiveDimensions.add(proceduralLayer);
            } else if (proceduralLayer.isFullyPrioritized) {
                higherNodeIsFullyOverriding = true;
            }
        }
        this.proceduralBlendMatrix.removeAll(inactiveDimensions);
    }
    ...
    private Set<String> getStatesInLayerStack() {
        ...
        this.proceduralBlendMatrix.forEach(proceduralLayer -> set.add(proceduralLayer.identifier));
        return set;
    }

    @Override
    public PoseFunction<LocalSpacePose> wrapUnique() {
        Builder builder = PoseManager.builder(this.initialStateFunction);
    ...
    public Optional<PoseFunction<?>> searchDownChainForMostRelevant(Predicate<PoseFunction<?>> findCondition) {
        ...
        // Search for a structural player parameter within the procedural matrix evaluation scope.
        for (ProceduralLayer proceduralLayer : this.proceduralBlendMatrix.reversed()) {
            var potentialPlayer = this.states.get(proceduralLayer.identifier).inputFunction.searchDownChainForMostRelevant(findCondition);
    ...
    // BFP ENGINE: Custom Procedural Structural Component
    private static class ProceduralLayer {
        private final String identifier;
        private final StateTransition entranceTransition;
        private final VariableDriver<Float> weight;
        private final float weightMathematicalDelta;
        private boolean isFullyPrioritized;

        private ProceduralLayer(String identifier, StateTransition entranceTransition) {
            this.identifier = identifier;
            this.entranceTransition = entranceTransition;
            this.weight = VariableDriver.ofFloat(() -> 0f);
            this.weightMathematicalDelta = 1f / Math.max(this.entranceTransition.transition().duration().inTicks(), 0.01f);
            this.isFullyPrioritized = false;
        }

        private void processMathematicalDampening() {
            this.weight.pushCurrentToPrevious();
            this.weight.modifyValue(currentValue -> Math.min(1f, currentValue + weightMathematicalDelta));
            if (this.weight.getCurrentValue() >= 1f && this.weight.getPreviousValue() >= 1f) {
                this.isFullyPrioritized = true;
            }
        }

        @Override
        public String toString() {
            return "ProceduralLayer{" + identifier + " W:" + weight.getCurrentValue() + "}";
        }
    }
    ...
    public static class Builder {
        ...
        public PoseFunction<LocalSpacePose> build(){
            ...
            return new PoseManager(this.states, this.initialState, this.resetUponRelevant, this.driversToUpdateOnStateChanged);
        }
    }
